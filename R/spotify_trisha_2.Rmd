...
temp1 <- lapply(seq_along(df0), function(x){
  ...
  curr_album <- df0[[x]]$master_metadata_album_album_name
  if(is.null(curr_album)){return(x)}
  curr_album <- paste0(curr_album, "___", curr_artist)
  if(albumToTime$containsKey(curr_album)){
    albumToTime$put(curr_album, albumToTime$get(curr_album) + df0[[x]]$ms_played)
  } else {
    albumToTime$put(curr_album, df0[[x]]$ms_played)
...
```{r}
colnames(dfb0)[1] <- "main"
dfb0$album <- lapply(dfb0$main, function(x){
  unlist(str_split(x, "___"))[1]
}) %>% unlist

dfb0$artist <- lapply(dfb0$main, function(x){
  unlist(str_split(x, "___"))[2]
}) %>% unlist

dfb0 <- dfb0[, c(4,5,2)]
colnames(dfb0)[3] <- "time_hours"

dfb0 <- dfb0[dfb0$time_hours >= 1, ]

write.table(dfb0, paste0(OUT_DIR, "most_listened_to_albums_lifetime.tsv"), sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
```
...
temp1 <- lapply(seq_down(df6), function(x){
  curr_track <- df6$spotify_track_uri[x]
  if(is.null(curr_track)){return(x)}
  trackToArtist$put(curr_track, df6$master_metadata_album_artist_name[x])
  return(0)
}) %>% unlist
dft$artist <- lapply(dft$track, function(x){ 
  if(trackToArtist$containsKey(x)){
    return(trackToArtist$get(x))
  }  else {
    return("NAN")
  }
} ) %>% unlist
