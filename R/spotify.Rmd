...
```{r}
library(jsonlite)
OUT_DIR <- "C:/Users/iwong1/Documents/mike/my_spotify_data_11/"
N_TOP_ARTIST <- 10
...
files <- list.files("C:/docs/mike/my_spotify_data_11/Spotify Extended Streaming History/", pattern = "json", full.names = TRUE)
...
plot(g1)
ggsave(paste0(OUT_DIR, "top_artists_2025.pdf"), g1, width = 10, height = 7, units = "in", limitsize = FALSE)
```
...
files <- list.files("C:/docs/mike/my_spotify_data_11/Spotify Extended Streaming History/", pattern = "Audio.*json", full.names = TRUE)
df0 <- lapply(files, function(x){read_json(x)}) %>% do.call(c, .)
df1 <- do.call(rbind, df0) %>% as.data.frame()
...
for(YEAR in MY_YEAR){
  ...
  g1 <- ggplot(df_heat, aes(x=Var3, y=y_time, fill=scaled_count_bin)) + 
    ...
    ggsave(paste0(OUT_DIR, "heatmap_listening_activity_week_", YEAR, ".pdf"), g2, width = 11, height = 9, units = "in", limitsize = FALSE)
}
```

comparison: disadvantage: 2d20 choose lowest? vs +2AC
AC vs 2d20 choose lowest?
can almost never be crit?
```{r}
AC <- 10:24
N <- 10000
temp1 <- data.frame(AC = AC, 
                    blur = lapply(AC, function(x){
  lapply(1:N, function(y){min(sample(1:20, 2, replace = TRUE)) >= (x-4)}) %>% unlist %>% mean
}) %>% unlist, faith = lapply(AC, function(x){
  lapply(1:N, function(y){sample(1:20, 1) >= (x-2)}) %>% unlist %>% mean
}) %>% unlist
)
temp1$choice <- ifelse(temp1$blur <= temp1$faith, "BLUR", "FAITH")
temp1$dif <- 100*(temp1$faith - temp1$blur)
