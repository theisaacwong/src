...
files <- list.files("C:/docs/mike/my_spotify_data_4//Spotify Extended Streaming History/", pattern = "json", full.names = TRUE)
...
library(plyr)
library(jsonlite)
OUT_DIR <- "C:/Users/iwong1/Documents/mike/my_spotify_data_4/"
TOP_N_ARTIST <- 4
...
files <- list.files("C:/docs/mike/my_spotify_data_4/Spotify Extended Streaming History/", pattern = "Audio.*json", full.names = TRUE)
df0 <- lapply(files, function(x){read_json(x)}) %>% do.call(c, .)
df1 <- do.call(rbind, df0) %>% as.data.frame()
