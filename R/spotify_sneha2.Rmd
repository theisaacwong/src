...
```{r}
library(jsonlite)
OUT_DIR <- "C:/Users/iwong1/Documents/mike/sneha/my_spotify_data-1/"
N_TOP_ARTIST <- 10
...
files <- list.files("C:/docs/mike/sneha/my_spotify_data-1/Spotify Extended Streaming History/", pattern = "json", full.names = TRUE)
...
g1 <- ggplot(dfa[1:N_TOP_ARTIST, ], aes(x=ID, y=time, fill=artist)) + 
  ...
  scale_y_continuous(breaks=seq(0, dfa$time[1]+50, 25))
plot(g1)
...
g1 <- ggplot(dfa0[1:N_TOP_ARTIST, ], aes(x=ID, y=time, fill=artist)) + 
  ...
  scale_fill_manual(name = "Artist", values = gg_color_hue(N_TOP_ARTIST) %>% my_rev) + 
  scale_y_continuous(breaks=seq(0, dfa0$time[1]+200, 25))
plot(g1)
...
files <- list.files("C:/docs/mike/sneha/my_spotify_data-1/Spotify Extended Streaming History/", pattern = "Audio.*json", full.names = TRUE)
df0 <- lapply(files, function(x){read_json(x)}) %>% do.call(c, .)
df1 <- do.call(rbind, df0) %>% as.data.frame()
...
ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "artists_by_year.pdf"), g1, width = 16, height = 10, units = "in", limitsize = FALSE)
g1
```
...
g1 <- ggplot(df_byyear, aes(x=year, y=time_hours, group=1)) + 
  ...
  scale_y_continuous(breaks=seq(0,120,2)) + 
  coord_cartesian(ylim=c(0, 30))
plot(g1)
ggsave(paste0(OUT_DIR, "listening_time_lifetime_by_year.pdf"), g1, width = 8, height = 8, units = "in", limitsize = FALSE)
```
...
my_plots <- lapply(my_years, function(this_year){
  ...
  g1 <- ggplot(df_filtered, aes(fill=master_metadata_album_album_name, y=hour, x = rowID, label = master_metadata_album_album_name)) + 
    ...
    theme(legend.position="none")
  ggsave(paste0(OUT_DIR, "top_albums_by_month_", this_year, ".pdf"), g1, width = 24, height = 14, units = "in", limitsize = FALSE)
  return(g1)
})
my_plots
```
...
my_plots <- lapply(my_years, function(this_year){
  ...
  g1 <- ggplot(df_filtered, aes(y=hour, x = rowID)) + 
    ...
    ggtitle(paste0("Spotify top ", TOP_N_ARTIST, " albums by month, ", this_year)) + 
    # scale_fill_manual(name = "Artist", values = gg_color_hue(df_filtered$master_metadata_album_album_name %>% lu) %>% my_rev) + 
    scale_y_continuous(breaks=seq(0, max(df_filtered$hour), 10)) +
    theme(legend.position="none")
  ggsave(paste0(OUT_DIR, "listening_time_monthly_total_", this_year, ".pdf"), g1, width = 12, height = 8, units = "in", limitsize = FALSE)
  return(g1)
})
my_plots
```
...
my_plots <- lapply(my_years, function(this_year){
  ...
  g1 <- ggplot(df_filtered, aes(fill=spotify_track_uri , y=hour, x = rowID, label = label )) + 
    ...
    theme(legend.position="none")
   ggsave(paste0(OUT_DIR, "top_songs_by_month_", this_year, ".pdf"), g1, width = 18, height = 12, units = "in", limitsize = FALSE)
  return(g1)
})
...
g1 <- ggplot(df_filtered, aes(fill=spotify_track_uri , y=hour, x = year, label = label )) + 
  ...
  theme(legend.position="none")
ggsave(paste0(OUT_DIR, "top_songs_by_year.pdf"), g1, width = 18, height = 7, units = "in", limitsize = FALSE)
g1
```
