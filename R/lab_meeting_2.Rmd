...
g6 <- ggplot(df02) + 
  geom_jitter(data=df02, aes(x = mID, y = diff_log, colour = rule), width = 0.25, height = 0.05, alpha=0.3) +
  geom_boxplot(data=df02, aes(x=mID, y = diff_log , group=super_group), alpha = 0.5, outlier.shape = NA) +
  scale_x_continuous(breaks = df02$mID %>% u, labels = df02$hostname %>% u) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + 
  xlab("hostname") +
...
ggsave("C:/docs/benchmark/pbsv/20250113/benchmark1-10.pdf", g6, width = 14, height = 8, units = "in", limitsize = FALSE)
```


```{r}
files <- list.files("C:/docs/benchmark/pbsv/20260204/", recursive = TRUE, full.names = TRUE, pattern="log")
...
ggsave("C:/docs/benchmark/pbsv/20260204/benchmark1.pdf", g3, width = 14, height = 8, units = "in", limitsize = FALSE)
ggsave("C:/docs/benchmark/pbsv/20260204/benchmark2.pdf", g4, width = 14, height = 8, units = "in", limitsize = FALSE)
ggsave("C:/docs/benchmark/pbsv/20260204/benchmark3.pdf", g6, width = 14, height = 8, units = "in", limitsize = FALSE)
```



```{r}
files <- list.files("C:/docs/benchmark/pbsv/20260204/results12/", recursive = TRUE, full.names = TRUE, pattern="log")
...
ggsave("C:/docs/benchmark/pbsv/20260204/benchmark1a.pdf", g3, width = 14, height = 8, units = "in", limitsize = FALSE)
ggsave("C:/docs/benchmark/pbsv/20260204/benchmark2a.pdf", g4, width = 14, height = 8, units = "in", limitsize = FALSE)
ggsave("C:/docs/benchmark/pbsv/20260204/benchmark3a.pdf", g6, width = 14, height = 8, units = "in", limitsize = FALSE)
```

```{r}
temp1 <- lapply(df01$age %>% u, function(x){
  data.frame(age = x, 
             sum = sum(df01$n[df01$age==x]))
}) %>% do.call(rbind, .)
```

```{r}
files <- list.files("C:/docs/benchmark/pbsv/20260401/results/", recursive = TRUE, full.names = TRUE, pattern="log")
...
ggsave("C:/docs/benchmark/pbsv/20260401/benchmark1a.pdf", g3, width = 14, height = 8, units = "in", limitsize = FALSE)
ggsave("C:/docs/benchmark/pbsv/20260401/benchmark2a.pdf", g4, width = 14, height = 8, units = "in", limitsize = FALSE)
ggsave("C:/docs/benchmark/pbsv/20260401/benchmark3a.pdf", g6, width = 14, height = 8, units = "in", limitsize = FALSE)
```
