...
df00 <- lapply(files, function(file){
  ...
  temp_collapsed <- lapply(temp_file$rule %>% u, function(x){
    ...
    diff <- as.numeric(end - starts,units="secs") %>% min
...
g3 <- ggplot(df01) + 
  ...
  geom_errorbar( aes(x=mID, ymin=cummean-sd, ymax=cummean+sd), width=0.4, colour="orange", alpha=0.9, linewidth=1.3) + 
  scale_x_continuous(breaks = df01$mID, labels = df01$hostname) +
  scale_y_continuous(breaks = my_ys, labels = my_ys/3600) + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + 
  xlab("hostname") + 
  ylab("time (hours)") + 
  geom_vline(data = df_vlines, aes(xintercept = x_val), linetype="dotted") + 
  geom_text(data=df_vlines, aes(label=hostname, x=x_lab, y=y_val), fontface = "bold") +
  ggtitle("time to run PBSV")
g3 
```

239 e001.grid.gs.washington.edu pbsv discover --tandem-repeats  1192 C:/docs/benchmark/pbsv/results2/f2923/log.txt
240 e221.grid.gs.washington.edu                  pbsv call BND    97 C:/docs/benchmark/pbsv/results2/f2923/log.txt
