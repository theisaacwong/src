...
```{r}
getExtension <- function(file) strsplit(file, ".", fixed=T)[[1]][-1]
df0 <- fread("C:/docs/website/ncdu_20250812_backups.tsv", header=FALSE) %>% as.data.frame()
# df0$folders <- df0$V1 %>% dirname %>% str_split("/")
df1 <- df0
# df1$V1 <- df1$V1 %>%  str_remove("^/nect/eichler")
...
# fwrite(df2, "C:/docs/website/backups_dir.tsv", col.names = TRUE, row.names=FALSE, quote=FALSE, sep="\t")
# df2 <- fread("C:/docs/website/backups_dir.tsv") %>% as.data.frame()
...
df6 <- data.frame(size = round(df5$size), 
                  dir = df5$dir, 
                  stringsAsFactors = FALSE)
write.table(df6, "C:/docs/website/backups_dir_filtered_100.tsv", sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
write.table(df6[order(df6$size, decreasing = TRUE), ], "C:/docs/website/backups_dir_filtered_100_sort.tsv", sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
```
