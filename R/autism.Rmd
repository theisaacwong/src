...
df0 <- fread("C:/docs/autism/paf_man.txt", header=FALSE) %>% as.data.frame()
df_paf1 <- data.frame(paf=df0$V1[grepl("_hap1", df0$V1)])
df_paf2 <- data.frame(paf=df0$V1[grepl("_hap2", df0$V1)])
df_paf1$sample <- df_paf1$paf %>% bname %>% str_remove("_hap.*")
df_paf2$sample <- df_paf2$paf %>% bname %>% str_remove("_hap.*")
...
# write.table(df_pro, "C:/docs/autism/concat_chrom_sib.tsv", sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
```


```{r}
df_col <- read.table("C:/docs/etc/my_colors.txt", header=FALSE, sep="\t")
