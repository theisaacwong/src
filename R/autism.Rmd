...
```{bash}
cat batch_2_samples.txt  | xargs -I {} sh -c "mkdir {}; cd {}; ln -s /net/eichler/vol26/7200/software/pipelines/hifiasm-smk/hifi/runlocal; cp ../hifiasm_trio.yaml .; cd .."
```
...
df0 <- lapply(list_by_sample, function(x){
  ...
  data.frame(sample = curr_sample, 
             PASS = correct_fofn & no_logs & fastas,
             fofn = correct_fofn,
             logs = no_logs, 
             fasta = fastas) %>% return()
}) %>% do.call(rbind, .)

df0$sample[df0$PASS] %>% write.table("C:/docs/autism/outs_succ.txt", col.names = FALSE, row.names = FALSE, quote=FALSE)
```
