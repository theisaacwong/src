...
find_fcs_outputs <- function(sample_id, hap){
  ...
  str_bpdp <- ifelse(grepl("_(p|s).$", sample_id), "dip", "bp")
  
  mypath <- paste0("assembly_qc/QC_results/contamination_screening/results/", sample_id, "_", hap, "/fcs_adaptor/cleaned_sequences/", sample_id, ".hifiasm.", str_bpdp, ".", hap, ".p_ctg.gfa.fasta")
  if(mypath %in% df6){return(mypath)}

  mypath <- paste0("assembly_qc_GROUP2/QC_results/contamination_screening/results/", sample_id, "_", hap, "/fcs_adaptor/cleaned_sequences/", sample_id, ".hifiasm.", str_bpdp, ".", hap, ".p_ctg.gfa.fasta")
  if(mypath %in% df7){return(mypath)}
  
  mypath <- paste0("assembly_qc/QC_results_old/contamination_screening/results/", sample_id, "_", hap, "/fcs_adaptor/cleaned_sequences/", sample_id, ".hifiasm.", str_bpdp, ".", hap, ".p_ctg.gfa.fasta")
...
```{r}
df_pbsv_issues <- df0[(df0$PBSV_GRCH38_BND=="" | df0$PBSV_GRCH38_DUP=="" | df0$PBSV_GRCH38_SV=="") & df0$aln_grch38!="",]
write(df_pbsv_issues[], "C:/docs/autism/asm_qc/pbsv_to_run.tsv")

df_pav_man <- df0[, c("sample", "asm_qc_hap1", "asm_qc_hap1")]
```
