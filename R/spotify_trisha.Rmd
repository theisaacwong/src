...
```{r}
library(jsonlite)
OUT_DIR <- "C:/Users/iwong1/Documents/mike/my_spotify_data trisha_extended/"
N_TOP_ARTIST <- 10
...
files <- list.files("C:/docs/mike/my_spotify_data trisha_extended/Spotify Extended Streaming History/", pattern = "json", full.names = TRUE)
...
g1 <- ggplot(dfa[1:N_TOP_ARTIST, ], aes(x=ID, y=time, fill=artist)) + 
  ...
  xlab("") + 
  ylab("Hours Listened") +
  ggtitle(paste0("Spotify top ", N_TOP_ARTIST, " Artists, 2024")) +
  scale_fill_manual(name = "Artist", values = gg_color_hue(N_TOP_ARTIST) %>% my_rev) + 
  scale_y_continuous(breaks=seq(0, dfa$time[1]+50, 5))
plot(g1)
ggsave(paste0(OUT_DIR, "top_artists_2024.pdf"), g1, width = 10, height = 7, units = "in", limitsize = FALSE)
```
...
g1 <- ggplot(dfa0[1:N_TOP_ARTIST, ], aes(x=ID, y=time, fill=artist)) + 
  ...
  scale_fill_manual(name = "Artist", values = gg_color_hue(N_TOP_ARTIST) %>% my_rev) + 
  scale_y_continuous(breaks=seq(0, dfa0$time[1]+100, 20))
plot(g1)
ggsave(paste0(OUT_DIR, "top_artists_lifetime.pdf"), g1, width = 10, height = 7, units = "in", limitsize = FALSE)


dft0$ID <- seq_down(dft0)
N_TOP_ARTIST <- 10
g1 <- ggplot(dft0[1:N_TOP_ARTIST, ], aes(x=ID, y=time, fill=track)) + 
  geom_bar(position="stack", stat="identity") +
  ...
  ggtitle(paste0("Spotify Top ", N_TOP_ARTIST, " Artists, lifetime")) +
  scale_fill_manual(name = "Artist", values = gg_color_hue(N_TOP_ARTIST) %>% my_rev) + 
  scale_y_continuous(breaks=seq(0, dft0$time[1]+100, 20))
plot(g1)
ggsave(paste0(OUT_DIR, "top_songs_lifetime.pdf"), g1, width = 10, height = 7, units = "in", limitsize = FALSE)
```
...
files <- list.files("C:/docs/mike/my_spotify_data trisha_extended/Spotify Extended Streaming History/", pattern = "Audio.*json", full.names = TRUE)
df0 <- lapply(files, function(x){read_json(x)}) %>% do.call(c, .)
df1 <- do.call(rbind, df0) %>% as.data.frame()
...
write.table(df3, paste0(OUT_DIR, "spotify_extended_history_trisha.tsv"), sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
```
...
my_plots <- lapply(my_years, function(this_year){
  ...
  g1 <- ggplot(df_filtered, aes(fill=master_metadata_album_artist_name, y=hour, x = rowID, label = master_metadata_album_artist_name)) + 
    ...
    scale_fill_manual(name = "Artist", values = gg_color_hue(df_filtered$master_metadata_album_artist_name %>% lu) %>% my_rev) + 
    scale_y_continuous(breaks=seq(0, max_hours, 25))
  ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "_artists_by_month_", this_year, ".pdf"), g1, width = 14, height = 12, units = "in", limitsize = FALSE)
  # return(g1)
  
  df_filtered <- df_filtered[df_filtered$master_metadata_album_artist_name != "Other", ]
  
  g2 <- ggplot(df_filtered, aes(fill=master_metadata_album_artist_name, y=hour, x = rowID, label = master_metadata_album_artist_name)) + 
    geom_bar(position="stack", stat="identity") +
    scale_x_continuous(breaks = df_filtered$rowID %>% u, labels = month.name[df_filtered$rowID %>% u]) + 
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), legend.position="none") + 
    geom_text(size = 3, position = position_stack(vjust = 0.5)) +
    ...
    scale_fill_manual(name = "Artist", values = gg_color_hue(df_filtered$master_metadata_album_artist_name %>% lu) %>% my_rev) + 
    scale_y_continuous(breaks=seq(0, max_hours, 25))
  ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "_artists_by_month_", this_year, "_no_other.pdf"), g2, width = 14, height = 12, units = "in", limitsize = FALSE)
  return(c(g1, g2))
})
my_plots
...
ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "_artists_by_year.pdf"), g1, width = 16, height = 14, units = "in", limitsize = FALSE)
g1
```
...
g1 <- ggplot(df_byyear, aes(x=year, y=time_hours, group=1)) + 
  geom_line() + 
  geom_point() + 
  ylab("Time (days)") + 
  xlab("Year") + 
...
plot(g1)
ggsave(paste0(OUT_DIR, "total_listening_time_by_year.pdf"), g1, width = 10, height = 6, units = "in", limitsize = FALSE)
```
...
my_plots <- lapply(my_years, function(this_year){
  ...
  g1 <- ggplot(df_filtered, aes(fill=master_metadata_album_album_name, y=hour, x = rowID, label = master_metadata_album_album_name)) + 
    ...
    theme(legend.position="none")
  ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "_albums_by_month_", this_year, ".pdf"), g1, width = 22, height = 10, units = "in", limitsize = FALSE)
  return(g1)
})
my_plots
```
...
my_plots <- lapply(my_years, function(this_year){
  ...
  g1 <- ggplot(df_filtered, aes(fill=spotify_track_uri , y=hour, x = rowID, label = label )) + 
    ...
    theme(legend.position="none")
   ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "_songs_by_month_", this_year, ".pdf"), g1, width = 14, height = 12, units = "in", limitsize = FALSE)
  return(g1)
})
...
g1 <- ggplot(df_filtered, aes(fill=spotify_track_uri , y=hour, x = year, label = label )) + 
  ...
  theme(legend.position="none")
ggsave(paste0(OUT_DIR, "top_", TOP_N_ARTIST, "_songs_by_year.pdf"), g1, width = 16, height = 12, units = "in", limitsize = FALSE)
g1
```
...
```{r}
df0 <- fread("C:/docs/mike/Renpho Health Trisha Ray", sep=",") %>% as.data.frame()
```
